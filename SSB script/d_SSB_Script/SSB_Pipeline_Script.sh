...
cat SampleList.txt | tail -n +2 | while read id;do bwa mem -t 40 -M -R '@RG\tID:foo\tSM:bar\tLB:library1' /data/Index/Gencode/Dolphin/GRCh37.p13 1_Filter/${id}_1.fq.trim 1_Filter/${id}_2.fq.trim > 2_AlignSam/${id}.sam;don